...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!page || page.category !== "supply") return {};

  const canonicalPath = `/supply/${page.slug}`;
...
export default async function SupplyPage({ params }: Props) {
  const { slug } = await params;
  const page = getCustomPageData(slug);
  if (!page || page.category !== "supply") notFound();

  return <LandingPageLayout page={page} categoryPath="supply" />;
}
