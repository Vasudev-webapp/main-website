...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!page || page.category !== "location") return {};

  const canonicalPath = `/location/${page.slug}`;
...
export default async function LocationPage({ params }: Props) {
  const { slug } = await params;
  const page = getCustomPageData(slug);
  if (!page || page.category !== "location") notFound();

  return <LandingPageLayout page={page} categoryPath="location" />;
}
